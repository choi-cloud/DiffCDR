...
def model_wrapper_hierarchical_cond(
    ...
    def model_fn(x, t_continuous):
        if is_cond_classifier:
            ...
            return model(x, t_discrete, cond_emb_quantized, cond_mask)
...
def hierarchical_cond_from_levels(all_level_vectors: torch.Tensor, t_continuous: torch.Tensor, noise_schedule, rq_num=0):
    ...
    # -------------------------------------------------
    if rq_num != 0:
        rq_num = min(max(int(rq_num), 1), L)

        gates = torch.zeros(L, B, device=device, dtype=all_level_vectors.dtype)
    ...
    # hard gate: [L, B]
    gates = (t_norm.unsqueeze(0) >= level_pos.view(L, 1)).to(all_level_vectors.dtype)
...
def model_wrapper(
    model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1.0, time_input_type="1", total_N=1000, model_kwargs={}
):
...
class DPM_Solver:
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N, device):
        ...
        if skip_type == "logSNR":
            ...
            return torch.flip(
                torch.cat([t[torch.searchsorted(quadratic_t, quadratic_steps)[:-1]], t_T * torch.ones((1,)).to(device)], dim=0), dims=[0]
            )
        else:
    ...
    def dpm_solver_first_update(self, x, s, t, return_noise=False):
        ...
        x_t = torch.exp(log_alpha_t - log_alpha_s)[(...,) + (None,) * dims] * x - (sigma_t * phi_1)[(...,) + (None,) * dims] * noise_s
        if return_noise:
    ...
    def dpm_solver_second_update(self, x, s, t, r1=0.5, noise_s=None, return_noise=False):
        ...
        if noise_s is None:
            noise_s = self.model_fn(x, s)
        x_s1 = torch.exp(log_alpha_s1 - log_alpha_s)[(...,) + (None,) * dims] * x - (sigma_s1 * phi_11)[(...,) + (None,) * dims] * noise_s
    ...
    def dpm_solver_third_update(self, x, s, t, r1=1.0 / 3.0, r2=2.0 / 3.0, noise_s=None, noise_s1=None, noise_s2=None):
        """
