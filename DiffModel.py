...
from dpm_solver_pytorch import NoiseScheduleVP, hierarchical_cond_from_levels
...
class DCB_CDR(nn.Module):
    def __init__(
        ...
        self.aggregation = parallel.get("aggregation", "aggregation")

        # RQVAE setting
        self.rqvae = rqvae

        self.step_mlp = nn.Sequential(SinusoidalPositionEmbeddings(self.input_dim))

        self.diff_models = nn.ModuleList()

        if self.aggregation in ["aggregation"]:
...
def diffusion_loss_fn_parallel(
    ...
    if is_task == False:
        ...
        mapping_loss = F.mse_loss(style_u, target_u_norm.detach())
        ...
        user_style_recon_loss = F.mse_loss(style_recon, target_u_norm.detach())
