...
class DiffParallel(nn.Module):
    def __init__(self,num_steps=200, diff_dim=32,input_dim =32,c_scale=0.1,diff_sample_steps=30,\
                 diff_task_lambda=0.1,diff_mask_rate=0.1, vbge_opt=None, use_vbge=True, parallel=None ):
        ...
        self.diff_models = nn.ModuleList([
                nn.ModuleList([ # diff model 1 -- MF condition 
                    nn.Linear(input_dim,diff_dim),    
                    nn.Linear(diff_dim,diff_dim) ,     
                    nn.Linear(diff_dim,input_dim),  
                ]),
                nn.ModuleList([ # diff model 2 -- VBGE condition 
                    nn.Linear(input_dim,diff_dim),    
                    nn.Linear(diff_dim,diff_dim) ,     
                    nn.Linear(diff_dim,input_dim),  
                ])
...
def diffusion_loss_fn(model,x_0,cond_emb, iid_emb,y_input, # DIM(reconstruction) loss 
                        device,is_task):
...
def diffusion_loss_fn_parallel(model,x_0_m,x_0_g,cond_emb1,cond_emb2, iid_emb,y_input, # DIM(reconstruction) loss 
                        device,is_task):
    ...
    if is_task == False: # DIM loss 먼저 
        ...
        if model.parallel['set_init'] == 0: # x_0 둘다 MF ui로  
            final_output_m, iid_emb=p_sample_loop_parallel(model,cond_emb1, cond_emb1,iid_emb,device, diff_id=0) # 디노이징 된 user emb_m / item emb
            final_output_g, iid_emb=p_sample_loop_parallel(model,cond_emb1, cond_emb2,iid_emb,device, diff_id=1) # 디노이징 된 user emb_g / item emb
        elif model.parallel['set_init'] == 1: # 각각 MF, Aggr 
            final_output_m, iid_emb=p_sample_loop_parallel(model,cond_emb1, cond_emb1,iid_emb,device, diff_id=0) # 디노이징 된 user emb_m / item emb
        ...
        if model.parallel['set_loss'] == 0:
            ...
            return F.smooth_l1_loss(x_0_m, final_output_m) + F.smooth_l1_loss(x_0_g, final_output_g) + model.task_lambda* task_loss # ALM 로스 + task loss
