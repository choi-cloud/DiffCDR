...
class MetaNet(torch.nn.Module):
    def __init__(self, emb_dim, meta_dim):
        super().__init__()
        self.event_K = torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim), torch.nn.ReLU(), torch.nn.Linear(emb_dim, 1, False))  # attention layer
    ...
    def forward(self, emb_fea, seq_index):
        ...
        his_fea = torch.sum(att * emb_fea, 1)  # p_{u_i}
        output = self.decoder(his_fea)  # w_{u_i}
...
class MFBasedModel(torch.nn.Module):
    ...
    def forward(self, x, stage):
        if stage == "train_src":
            ...
            uid_emb = torch.bmm(uid_emb_src, mapping)  # u_i^t = f(u_i^s, w_{u_i})
...
class Tenc(nn.Module):
    def __init__(self, hidden_size, emb_dim, dropout, diffuser_type, maxlen_source, device, num_heads=1):
        ...
        self.att = torch.nn.Sequential(
            torch.nn.Linear(self.maxlen_source * self.emb_dim, self.emb_dim * 2), torch.nn.ReLU(), torch.nn.Linear(self.emb_dim * 2, self.emb_dim)
        )
        ...
        self.emb_mlp = nn.Sequential(nn.SiLU(), nn.Linear(self.hidden_size, self.hidden_size * 2))
        ...
        if self.diffuser_type == "mlp1":
            self.diffuser = nn.Sequential(nn.Linear(self.hidden_size * 3, self.hidden_size))
        elif self.diffuser_type == "mlp2":
            self.diffuser = nn.Sequential(
                nn.Linear(self.hidden_size * 3, self.hidden_size * 2), nn.GELU(), nn.Linear(self.hidden_size * 2, self.hidden_size)
            )
...
class DiffBaseModel(torch.nn.Module):
    def __init__(
        ...
        self.diffusion = diffusion(timesteps, beta_start, beta_end, w, device)
        self.cacu_uidemb = uidEmbedding(uid_all, emb_dim)
        self.cacu_iidemb = iidEmbedding(iid_all, emb_dim)

    def forward(self, x, stage, timesteps=50, device="cuda"):
        if stage == "train_dmcdr":
            x_start = self.cacu_uidemb(x[:, 0])
            batch_size = x_start.shape[0]

            item_source = x[:, 2:]
            item_source = self.cacu_iidemb(item_source)
            item_target = self.cacu_iidemb(x[:, 1])  # (B, D)
            h = self.guided.cacu_h(item_source, x[:, 2:])

            n = torch.randint(0, timesteps, (batch_size,), device=device).long()
            predicted_x, loss_1 = self.diffusion.p_losses(self.guided, x_start, h, n, loss_type="l2")
            predicted_x = predicted_x.unsqueeze(1)  # (B, 1, D)
            ...
            item_target = item_target.unsqueeze(1)
            emb = torch.cat([predicted_x, item_target], 1)
        ...
        if stage == "test_dmcdr":
            x_t = self.cacu_uidemb(x[:, 0])
            item_source = x[:, 2:]
            item_source = self.cacu_iidemb(item_source)
            item_target = self.cacu_iidemb(x[:, 1])
            predicted_x = self.guided.predict(item_source, x[:, 2:], self.diffusion, x_t)
