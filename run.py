...
class Run:
    def __init__(self, config):
        ...
        self.dcb_cdr_mode = config.get("dcb_cdr_mode", "mean")
        parallel_weights = config.get("dcb_cdr_weights", [0.5, 0.5])
        ...
        self.dcb_cdr_weights = parallel_weights
    ...
    def get_data(self):
        ...
        graph_src_train = self.build_graph_inputs(self.src_path)
        graph_tgt_train = self.build_graph_inputs(self.tgt_path, exclude_users=test_users)

        graph_src_test = graph_src_train
        graph_tgt_test = graph_tgt_train
    ...
    def get_optimizer(self, model, diff_model=None, ss_model=None, la_model=None):
        ...
        elif diff_model is not None and isinstance(diff_model, Diff.DCB_CDR):
    ...
    def DCB_CDR(
        ...
        for i in range(self.epoch):
            ...
            loss, task_loss = self.train(
                data_diff, [model, diff_model], None, optimizer, i, stage="train_dcb_cdr", mapping=False, diff=True, style_src=style_src
            )

            mae, rmse = self.eval_mae([model, diff_model], data_test, stage="test_dcb_cdr", style_src=style_src)
            self.update_results(mae, rmse, "dcb_CDR")
    ...
    def _pretrain_single_quantizer(self, name, quantizer, embed_fetch_fn, data_diff):
        ...
        # --------------------------------------------------
        # union 전체(self.uid_all)가 아니라, 실제 source 도메인에
        # 인터랙션이 존재하는(=학습된) uid만 사용
        # --------------------------------------------------
        
        uid_set = set()
        for X, _ in data_diff:
            uid_set.update(X[:, 0].detach().cpu().long().tolist())

        all_uids = torch.tensor(sorted(uid_set), dtype=torch.long, device=self.device)
        write(f"{name}: pretraining on {len(all_uids)} source-domain users (out of {self.uid_all} total uid space)")
    ...
    def main(self, exp_part="None_CDR", save_path=None):
        ...
        elif exp_part == "dcb_CDR":

            diff_model = Diff.DCB_CDR(
                self.diff_steps,
                self.diff_dim,
                self.emb_dim,
                self.diff_scale,
        ...
        # --------------------------------------------------
        # Source User Rating Style Extraction
        # --------------------------------------------------
        print("\nExtracting source-domain user rating styles...\n")
        ...
        # --------------------------------------------------
        # Target Item Rating Style Extraction
        # --------------------------------------------------
        print("\nExtracting target-domain item rating styles...\n")
        ...
        # --------------------------------------------------
        # Target User Rating Style Extraction
        # --------------------------------------------------
        print("\nExtracting target-domain user rating styles...\n")
        ...
        if exp_part == "None_CDR":
            ...
            self.result_print(["dcb_CDR"], exp_part)
