...
class Run():
    ...
    def build_graph_inputs(self, data_path, include_users=None, exclude_users=None):
        ...
        uv_adj = torch.sparse_coo_tensor(uv_indices, edge_values,
                                        size=(self.uid_all, self.iid_all + 1)).coalesce()

        vu_indices = torch.stack([item_ids, user_ids])
        vu_adj = torch.sparse_coo_tensor(vu_indices, edge_values,
                                        size=(self.iid_all + 1, self.uid_all)).coalesce()
    ...
    def get_data(self): # 데이터로더 생성하고 학습 단계별로 재사용. 
        ...
        def _print_graph_stats(name, graph):
            if graph is None:
                ...
                print('{} edges: {} , unique users: {}, unique items: {}'.format(
    ...
    def Diff_Parallel(self, model,diff_model,  data_diff, data_test, optimizer, graph_train, graph_test):
        write('=========Diff_Parallel========')
        for i in range(self.epoch):
            loss,task_loss = self.train(data_diff,[model,diff_model],None,optimizer,i,stage='train_diff_parallel' , mapping=False, diff=True, graph_train=graph_train)
    ...
    def main(self, exp_part = 'None_CDR',save_path=None):
        ...
        if exp_part == 'diff_CDR':
            ...
            diff_model = Diff.DiffParallel(self.diff_steps, self.diff_dim, self.emb_dim,self.diff_scale,self.diff_sample_steps,self.diff_task_lambda,self.diff_mask_rate, self.vbge_opt, use_vbge=self.use_vbge, parallel=self.parallel_setting )
            diff_model = diff_model.cuda() if self.use_cuda else diff_model
